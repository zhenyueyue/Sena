...
## First accepted working base

The first exported candidate, `sena_base_01.vrm`, passed the structural gate
and is now the working foundation for Sena. The first Blender stylization pass
is generated by:

~~~powershell
./tools/blender/build_sena_base_chibi.ps1
~~~

It preserves the original VRoid Face/Body/Hair topology, skinning and facial
shape keys while applying the first chibi proportion pass:

- compressed body/leg height;
- larger head mass;
- narrower lower face;
- slightly larger anime eyes;
- shorter arm span;
- full 17/17 required humanoid bone coverage.

Generated review files stay under
`pets/sena/models/generated/base_chibi_01/` and are not committed.
